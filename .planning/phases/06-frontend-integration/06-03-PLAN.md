...
<task type="auto">
  <name>Task 2: Build full state-driven tab screen</name>
  <files>frontend/lib/screens/after_hours_tab_screen.dart</files>
  <action>
Replace the placeholder tab screen with full state-driven UI:
...
Also add `resetToInactive()` method to AfterHoursService if not already present:
```dart
