...
6. Methods:
   ...
   - `resetToInactive()` - resets state to inactive, clears session data (for expired -> start new)
...
9. resetToInactive() implementation:
   ```dart
...
Follow existing SocketService and SubscriptionService patterns exactly. Use debugPrint for logging.
  ...
  <verify>
    ...
    - Methods refreshSessionStatus() and resetToInactive() exist
  </verify>
  <done>AfterHoursService class exists with state machine, socket subscriptions, AfterHoursMatch model, and reset methods</done>
</task>
...
1. Add imports:
   ...
   Note: AfterHoursChatService already exists in frontend/lib/services/after_hours_chat_service.dart (created in Phase 5 Plan 05-02).
...
Note: ChangeNotifierProxyProvider2 is used because AfterHoursService depends on both AuthService AND SocketService.
  ...
  <verify>
    ...
    - AfterHoursChatService import resolves (file exists from Phase 5)
  </verify>
...
- AfterHoursService has refreshSessionStatus() and resetToInactive() methods
...
- AfterHoursChatService registered (import resolves to existing file from Phase 5)
