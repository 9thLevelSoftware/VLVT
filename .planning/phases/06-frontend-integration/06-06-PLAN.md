...
must_haves:
  ...
  key_links:
    - from: "frontend/lib/services/after_hours_service.dart"
      to: "/api/v1/after-hours/session and /api/v1/after-hours/match"
      via: "HTTP POST/GET/DELETE requests"
      pattern: "_url\\('/after-hours/(session|match)"
...
# All endpoints are prefixed with /api/v1 by the service, and /after-hours by the router
# The _url() helper builds paths like: AppConfig.profileUrl('/after-hours/session/start')
# which resolves to: https://<profile-service>/api/v1/after-hours/session/start
#
# Endpoints:
# - POST /after-hours/session/start - Start session (body: duration, latitude, longitude)
# - POST /after-hours/session/end - End session early
# - GET /after-hours/session - Get current session status
# - POST /after-hours/match/decline - Decline match (body: matchId)
...
Verify all API-related TODOs are removed:
```bash
# These specific TODOs should no longer exist:
grep -n "TODO.*API call to start" frontend/lib/services/after_hours_service.dart  # Should return nothing
grep -n "TODO.*API call to end" frontend/lib/services/after_hours_service.dart    # Should return nothing
grep -n "TODO.*accept match" frontend/lib/services/after_hours_service.dart       # Should return nothing
grep -n "TODO.*decline match" frontend/lib/services/after_hours_service.dart      # Should return nothing
grep -n "TODO.*refresh.*status" frontend/lib/services/after_hours_service.dart    # Should return nothing

# Count remaining TODOs (may have non-API TODOs, that's fine):
grep -c "TODO" frontend/lib/services/after_hours_service.dart  # 0 API-related TODOs expected
```
  </verify>
  <done>
...
All 5 API-related TODO stubs have been replaced with working HTTP code.
  </done>
</task>
...
2. Add ChangeNotifierProxyProvider for AfterHoursProfileService to the afterHours() method.
   Insert as the FIRST element in the list returned by afterHours() (before the existing AfterHoursService provider), since profile service may be needed first:

   ```dart
   static List<SingleChildWidget> afterHours() => [
     // NEW: Insert this as the first element in the list
     ...
     // EXISTING: Keep the existing AfterHoursService provider after it
...
```
Expected: Should find both the import line and the ChangeNotifierProxyProvider registration.
  </verify>
  <done>
...
Verify foreground task integration:
```bash
grep -n "FlutterForegroundTask" frontend/lib/services/after_hours_service.dart
```
Expected: Should find import, initForegroundTask, _startForegroundService, and _stopForegroundService.

Verify start/stop integration:
```bash
grep -n "_startForegroundService\|_stopForegroundService" frontend/lib/services/after_hours_service.dart
```
Expected: Should find method definitions AND calls in startSession() and resetToInactive().
  </verify>
  <done>
FlutterForegroundTask is initialized with proper Android notification options.
Foreground service starts automatically when a session begins (after successful API call).
Foreground service stops automatically when session ends (in resetToInactive).
