...
must_haves:
  truths:
    ...
    - "User knows how long they have to respond to match"
...
Uses the `_isMatchCardShowing` boolean flag (declared in Plan 06-03 Task 2a) to prevent duplicate modals instead of relying on the fragile ModalRoute.of(context)?.isCurrent pattern.
...
3. Add method:
   ...
   void _onServiceStateChanged() {
     ...
     if (service.state == AfterHoursState.matched && service.currentMatch != null) {
       // Use boolean flag to prevent duplicate modals (more reliable than ModalRoute.of)
...
7. Update the matched state case in _buildStateContent to be a placeholder (since overlay shows):
  ...
  <verify>
    ...
    - When state becomes matched, modal overlay appears (only once, no duplicates)
    - Swiping right calls acceptMatch
    - Swiping left calls declineMatch
    - Tapping Chat/Decline buttons works
    - Modal closes cleanly on accept/decline
  </verify>
  <done>Match card overlay integrated into tab screen using boolean flag to prevent duplicates</done>
</task>
...
8. Auto-decline timer visible and counts down (user knows how long they have to respond)
9. Verify modal doesn't show duplicate (trigger match event twice, only one modal appears)
</verification>
...
- Auto-decline timer visible if autoDeclineAt set (user knows response deadline)
- Modal cannot be dismissed by tapping outside or default drag
- Haptic feedback on swipe completion
- Boolean flag prevents duplicate modals (no ModalRoute fragility)
</success_criteria>
