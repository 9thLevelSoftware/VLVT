...
export async function initializeSessionWorker(pool: Pool): Promise<void> {
  ...
  redisConnection.on('connect', () => {
    logger.info('Redis connected for session scheduler');
  });
...
Exports scheduleSessionExpiry, cancelSessionExpiry, extendSessionExpiry, initializeSessionWorker, closeSessionScheduler
  ...
  <done>
BullMQ session scheduler service created with schedule, cancel, extend, and graceful shutdown functions
  </done>
</task>
...
After pool initialization, add:
```typescript
// Initialize session scheduler (BullMQ)
initializeSessionWorker(pool).catch(err => {
  logger.error('Failed to initialize session scheduler', { error: err.message });
  // Non-fatal: sessions will still work, expiry jobs may not schedule
});
```
...
```
  ...
  <verify>
TypeScript compiles: `cd backend/profile-service && npx tsc --noEmit`
Server starts: `cd backend/profile-service && npm run dev` (check for Redis connection logs)
Session endpoints exist in router
  </verify>
  <done>
Session lifecycle endpoints implemented: start, end, extend, status. BullMQ worker initialized on startup with graceful shutdown.
  </done>
</task>
...
3. Server starts and connects to Redis (or logs error if Redis unavailable)
