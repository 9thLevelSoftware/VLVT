...
<task type="auto">
  <name>Task 1: Add preferences validation chains</name>
...
File exports validatePreferences and validatePreferencesUpdate
  ...
  <done>
Preferences validation chains added with seeking gender, distance, age range, and orientation validation
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Add preferences endpoints to the existing After Hours router:
...
Note: The after_hours_preferences table needs min_age, max_age columns. Check if migration 021 has them; if not, add them in this task via ALTER TABLE or document for a follow-up migration.

IMPORTANT: Looking at migration 021, the table only has: user_id, seeking_gender, max_distance_km, interests. We need to add min_age, max_age, sexual_orientation columns.

Add a mini-migration at the top of the router file as a startup check, OR create a new migration file. Prefer the migration file approach:

Create `backend/migrations/022_add_after_hours_preferences_columns.sql` with:
```sql
...
Server starts without errors
  ...
  <done>
Preferences POST/GET/PATCH endpoints implemented with smart defaults and schema migration for new columns
  </done>
</task>
...
2. Migration 022 exists and runs successfully
