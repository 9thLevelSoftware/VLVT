...
5. **Update publishMatchToBothUsers to schedule auto-decline:**
   In the publishMatchToBothUsers function (created in 03-02), after publishing match events and before returning, add:
   ```typescript
...
New exports are accessible: scheduleAutoDecline, cancelAutoDecline
  ...
  <done>Auto-decline job scheduled when match created, fires after 5 minutes, notifies both users via Redis pub/sub and triggers re-matching.</done>
</task>
...
5. Auto-decline notifies both users via Redis pub/sub and triggers re-matching
</verification>

<success_criteria>
- Auto-decline job fires 5 minutes after match creation
- Auto-decline marks match with declined_by='system'
- Both users receive after_hours:match_expired event via Redis pub/sub (delivered by chat-service)
